...
template: "<button type='button' style='padding: 0.15em 0em;'  class='ui-datepicker-trigger ui-button ui-corner-all ui-widget yearPicker'><i class='fa fa-calendar fa-fw'></i></button>"+
...
const render = {
  renderYear: function renderYear() {
    ...
    $this.yearsCurrent.html(selectedYear == null ? thisYear : selectedYear);
...
const Yearpicker = (function () {
  function YearPicker(element, options) {
    ...
    $($this.element).next(".yearPicker").click(function(){
    	if ($(".yearPicker").prev().attr("disabled") != 'disabled') {
			if ($this.$template.hasClass($this.options.hideClass)) {
				$this.show = true;
				$this.$template.removeClass($this.options.hideClass).on(event_click, $.proxy($this.click, $this));
			}else{
				$this.show = false;
				$this.$template.addClass($this.options.hideClass).on(event_click, $.proxy($this.click, $this));
			}
    	}
	 });
...
//      `<${itemTag} class="${items.view} ${classes.join(
//        " "
