...
/*var urlCheck = getParameter(window.parent.document.location.href)
function getParameter(strParamName) {
    var tmpParam = strParamName.split("?");
    if (tmpParam[1]) {
        return tmpParam[1]
    }else{
        return '';
...
/*function testtttt(elem){
    var newWin = window.open(elem.href, 'ot');  
    setTimeout(function()
    {
            var wrap = $(newWin.document).find('head');
            wrap.append('<link rel="stylesheet" type="text/css" href="/css/en/ele/test.css">')
    }, 100) ;
}*/
...
function dTree(objName, data) {
    ...
    this.icon = {
        popup               : './guide/images/icon_popup.gif',
        root                : './guide/images/base.gif',
        folder          : './guide/images/folder.gif',
        folderOpen  : './guide/images/folderopen.gif',
        node                : './guide/images/page.gif',
        empty               : './guide/images/empty.gif',
        line                : './guide/images/line.gif',
        join                : './guide/images/join.gif',
        joinBottom  : './guide/images/joinbottom.gif',
        plus                : './guide/images/plus.gif',
        plusBottom  : './guide/images/plusbottom.gif',
        minus               : './guide/images/minus.gif',
        minusBottom : './guide/images/minusbottom.gif',
        nlPlus          : './guide/images/nolines_plus.gif',
        nlMinus         : './guide/images/nolines_minus.gif'
    };
    this.obj = objName;
    this.aNodes = [];
    this.aIndent = [];
...
dTree.options = {
    statusColors: {
        "대기": '#fff;background:#ccc',
        "기본": '#999',
        "검수요청": '#9b0bff',
        //"스크립트": "brown",
       // "스크립트검수": "#40724b",
        "완료": "#0042ff;font-weight:bold",
        "수정": "#ff0000",
        "delete": "delete"
    },
    target: '',
    domain : document.location.origin + ((document.location.pathname.indexOf('html/hmall_publish') !== -1) ? 'html/hmall_publish' : ''),
    imgDir : ((document.location.pathname.indexOf('html/hmall_publish') !== -1) ? '/html/hmall_publish' : '')+"/guide/script_guide/guide/images/"
};
...
dTree.prototype.node = function(node, nodeId) {
    ...
    var domain = (dTree.options.domain)? dTree.options.domain : '';
    ...
    var color = '';
    if (node.title) {
        if(dTree.options.statusColors[node.title]){ 
            node.title = dTree.options.statusColors[node.title]; 
        }
        if (dTree.options.statusColors[node.title] == 'delete'){
            console.log('fsd')
            color += ' style="background:#999;color:' + node.title + '"';
        }else{
    ...
    if (node.url) {
        /*if (urlCheck.indexOf('dev') !== -1 && urlCheck.indexOf('safari') !== -1) {
            node.url +='?dev=true&safari=true';
        }else if(urlCheck.indexOf('dev') !== -1){
            node.url +='?dev=true';
        }else if(urlCheck.indexOf('safari') !== -1){
            node.url +='?safari=true';
        };*/
        ...
        if (node.name.indexOf('완료') !== -1) {
            str += '<a href="javascript: ' + this.obj + '.o(' + nodeId + ');" class="node complete" '+color+'>';
        }else{
