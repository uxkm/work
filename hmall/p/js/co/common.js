...
;(function(core, $, undefined){
    ...
    Scrollspy = Widget.extend({
        ...
        _setNavHeight : function(){
            ...
            console.log("######",innerCont.outerHeight(),_.options.navHeight);
...
$.fn.scrollManager = function(option){
    ...
    // <!-- public method start
    /*
        disable : selector의 scroll, resize 이벤트 발생시 callback 실행을 막고
        scrollbar를 감춥니다.
    */
    ...
    /*
        enable : selector의 scroll, resize 이벤트 발생시 callback 실행을 막았던 것을
        풀고 scrollbar를 다시 보이게 합니다.
    */
   this.enable = function(){
    ...
    /*
        scroll시 custom event를 발생시킵니다. customScroll + '.네임스페이스'
        direction(상하 스크롤 정보)를 포함한 event객체를 param으로 보내줍니다. 
    */
    ...
    /*
        resize시 custom event를 발생시킵니다. customResize + '.네임스페이스'
        200ms 간격으로 event throttle이 구현되어있습니다.
    */
...
//     // <!-- public method start
//     /*
//         initPosition : sticky될때 붙어있어야할 위치를 재계산합니다. window가 resize 될때만
//         해두면 됩니다.
//     */
...
//             $selector.removeClass('fixed').removeAttr('style')//일시적으로 fixed를 해제
//             var rect = target.getBoundingClientRect() //화면상의 절대 위치 반환
//             var computedStyle = getComputedStyle(target) //style값 구하기

//             //style값 기준으로 margin,padding 구하기
...
//             if(computedStyle.transform){//transform으로 변형된 값 구하기
...
//         })
//         _self.stick() //구한 값으로 다시 sticky 여부 판단
//     }

//     /*
//         stick : 현재 window의 scrollTop값을 구하고 각 개체들의 sticky되어야하는지 판별합니다.
//     */
//     this.stick = function(){
...
$.fn.layerPopTrigger = function(option){
    ...
    return this.each(function(i, e){
        ...
        var $target = $('.layer-popup'+'.'+target).eq(0) //하나만 선택
...
$(function(){
    ...
    /* 공통 categoryNavigation 구현 start */
    if($('.category-area').length !== 0){
        UI.categoryNavigation = $('.category-area').categoryNavigation()
    }
    
    /* 공통 categoryNavigation 구현 end */

    /* 공통 layerPopTrigger 구현 start */
